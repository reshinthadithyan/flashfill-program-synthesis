...
class CodeT5(pl.LightningModule):
    ...
    def __init__(
        self, lr=5e-5, num_train_epochs=args.max_epochs, warmup_steps=args.warmup_steps
    ):
        super().__init__()
        self.model = T5ForConditionalGeneration.from_pretrained(args.model_name)
        tokenizer = RobertaTokenizerFast.from_pretrained(args.model_name)
        self.trainset, self.validset, self.testset = train_test_valid_split(
            "FlashFill", tokenizer, args.target_max_length
        )

...
class FlashFillDataset(Dataset):
    ...
    def read_and_process(self,path=os.path.join(".","DeepSynth","flashfill_dataset")):
        """
        ...
        if self.dataset_idt == "FlashFill":
            ...
            for file in tqdm(file_names):
                with open(file,"r") as f:
                    ...
                    model_inputs = self.tokenizer(inputs, max_length=self.target_length, padding="max_length", truncation=True)
                    labels = self.tokenizer(to_generate, max_length=self.target_length, padding="max_length", truncation=True).input_ids
...
def train_test_valid_split(dataset_idt,tokenizer,max_length):
    ...
    return random_split(flashfill_dataset,[92,8,8], generator=Generator().manual_seed(42))
...
if __name__ == "__main__":
    ...
    print(random_split(flashfill_dataset,[92,8,8], generator=Generator().manual_seed(42)))
